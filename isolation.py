...
class Board:
    ...
    def set_state(self, board_state, p1_turn=True):
        ...
        for rook in self.__rooks__:
            ...
            if last_move:
                # set last move to the first found occurrence of the given Rook͏︌͏󠄁͏︉
        ...
        # Count X's to get move count + 2 for initial moves͏︌͏󠄁͏︉
        self.move_count = sum(row.count('R1') + row.count('R3') + row.count('R2') + row.count('R4') for row in board_state)

    #function to edit to introduce any variant - edited for castle isolation variant by Abirath Raju (2/08/2024)͏︌͏󠄁͏︉
    def __apply_move__(self, rook_move):
        ...
        '''
        # print("Applying move:: ", rook_move)͏︌͏󠄁͏︉
        rook, row, col = rook_move
        my_pos = self.__last_rook_move__[rook]
        #opponent_pos = self.__last_rook_move__[self.__inactive_players_rook__]͏︌͏󠄁͏︉

        ######Change the following lines to introduce any variant######͏︌͏󠄁͏︉
        if my_pos != Board.NOT_MOVED:
            self.__board_state__[my_pos[0]][my_pos[1]] = Board.BLOCKED

            #check if rook moves more than 1 space in any direction͏︌͏󠄁͏︉
            # if abs(col - my_pos[0]) > 1 or abs(row - my_pos[1]) > 1:͏︌͏󠄁͏︉
            #     self.__create_crater__(rook_move)͏︌͏󠄁͏︉
       ######Change above lines to introduce any variant######͏︌͏󠄁͏︉

        # apply move of active player͏︌͏󠄁͏︉
        self.__last_rook_move__[rook] = (row,col)
        self.__board_state__[row][col] = self.__rook_symbols__[rook]


        # rotate the players͏︌͏󠄁͏︉
        self.__active_player__, self.__inactive_player__ = self.__inactive_player__, self.__active_player__

        # rotate the rooks͏︌͏󠄁͏︉
        self.__active_players_rook__, self.__inactive_players_rook__ = self.__inactive_players_rook__, self.__active_players_rook__

        # increment move count͏︌͏󠄁͏︉
        self.move_count = self.move_count + 1

        # If opponent is isolated͏︌͏󠄁͏︉
    ...
    def __get_moves__(self, move):
        ...
        for direction in directions:
            for dist in range(1, max(self.height, self.width)):
                ...
                # Allow for wrapping moves to other side of the board͏︌͏󠄁͏︉
                # if col < 0: col += self.height͏︌͏󠄁͏︉
                # if col >= self.height: col -= self.height͏︌͏󠄁͏︉
                # if row < 0: row += self.width͏︌͏󠄁͏︉
                # if row >= self.width: row -= self.width͏︌͏󠄁͏︉
    ...
    def play_isolation(self, time_limit=10000, print_moves=False):
        ...
        while True:
            ...
            def time_left():
                # print("Limit: "+str(time_limit) +" - "+str(curr_time_millis()-move_start))͏︌͏󠄁͏︉
            ...
            if is_over:
                ...
                # if not self.get_active_moves():͏︌͏󠄁͏︉
                #     return self.__active_players_rook__, move_history, \͏︌͏󠄁͏︉
                #            (self.__inactive_player__.__class__.__name__ + " has no legal moves left.")͏︌͏󠄁͏︉
                # return self.__active_player__.__class__.__name__, move_history, \͏︌͏󠄁͏︉
                #        (self.__inactive_player__.__class__.__name__ + " was forced off the grid.")͏︌͏󠄁͏︉
